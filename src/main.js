...
function getSemanticType(value, types, fields, key) {   
  if (value === true || value === false) {
    return types.BOOLEAN;
  } else if (value != null && value !== '' && value !== "" && !isNaN(value)) {
    return types.NUMBER;
...
function createField(fields, types, key, value) {
  var semanticType = getSemanticType(value, types, fields, key);
  ...
  if(semanticType == types.YEAR_MONTH_DAY_SECOND) {
...
function formatColumnValue(value, fieldType){
  var types = cc.FieldType;
  //console.log("formatColumnValue - " + value + " - " + fieldType);
...
function getColumns(content, requestedFields) {
  ...
  return content.map(function (row) {
    ...
    requestedFields.asArray().forEach(function (field) {
      ...
      var fieldValue = row === null ? '' : formatColumnValue(getColumnValue(valuePaths, row), field.getType());
...
function getData(request) {
  ...
  var rows = getColumns(content, requestedFields);
